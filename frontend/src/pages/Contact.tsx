...
const contactItems: {
  ...
  { icon: 'map-pin', label: 'Office Address', value: profile.address },
  ...(profile.phone
  ...
  ...(profile.social.instagram
    ? [{ icon: 'instagram' as const, label: 'Instagram', value: '@rajeevjaitly', href: profile.social.instagram }]
    : []),
  {
    icon: 'clock',
    label: 'Office Hours',
        ...
        <div className="max-w-6xl mx-auto px-4">
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden mb-6 flex flex-col sm:flex-row">
            <img
              src={images.profile}
              alt={profile.name}
              className="sm:w-48 md:w-56 h-36 sm:h-auto object-cover object-top shrink-0"
            />
                ...
                {contactItems.map((item) => (
                  <Card key={item.label} className="!p-4 flex gap-3 items-start">
                    <div className="w-10 h-10 rounded-lg bg-orange-100 flex items-center justify-center shrink-0">
                      <Icon name={item.icon} className="text-orange-600" size={20} />
                    </div>
                    <div className="min-w-0">
                      ...
                      {item.href ? (
                        <a
                          ...
                          className="text-gray-800 text-sm font-medium hover:text-orange-600 break-all"
                        >
                          {item.value}
                        </a>
                      ) : (
              ...
              <Card className="h-full">
                <div className="flex items-center gap-3 mb-5 pb-4 border-b border-gray-100">
                  <div className="w-10 h-10 rounded-lg bg-orange-100 flex items-center justify-center">
                    <Icon name="message-square" className="text-orange-600" size={20} />
                  </div>
                  <div>
                ...
                {submitted ? (
                  <div className="text-center py-10">
                    <div className="w-14 h-14 bg-orange-100 rounded-full flex items-center justify-center mx-auto mb-4">
                      <Icon name="send" className="text-orange-700" size={24} />
                    </div>
